...
The MCP protocol track below no longer claims a major of its own. It lands across the **v3.x** line as fastmcp support arrives, so nothing collides with this number.
...
The MCP spec **shipped GA on 2026-07-28** ([announcement](https://blog.modelcontextprotocol.io/posts/2026-07-28/)). Protocol-level work is now gated only on fastmcp shipping support for it; the goal is a single coordinated protocol release rather than dribbling breaking changes across patches. That coordinated release now lands inside the **v3.x** line rather than claiming a major of its own. It was scoped as a standalone v3.0 major until the engine swap took that number, and the decision since is to fold it into v3.x instead of reserving v4.0 for work whose timing is set by a dependency.

**v3.x scope (gated on fastmcp v4):**
...
**Later v3.x (post-spec GA, gated on host adoption):**
...
**Last Updated:** 2026-08-23 (next cut decided as v3.0.0; MCP protocol track folded into v3.x, no v4.0 reserved)
