...
- **Fix `total_matches` schema inconsistency in `pdf_search`** (from 1.11.0 LLM evaluation) — In hybrid (`auto`) mode, `total_matches` reflects the pre-fusion keyword count, not the actual returned match count, so responses ship `matches: [5 items]` alongside `total_matches: 0`. In `semantic` mode, the field is omitted entirely. Fix: recompute `total_matches` (and `page_match_counts`) from `hybrid_matches` after RRF fusion, and add `total_matches` to the semantic-mode return dict for cross-mode schema parity. Two-line change in `server.py`; highest-impact LLM-visible bug.
- **Tokenised keyword search** (from 1.11.0 LLM evaluation) — Page-grain `pdf_search` keyword mode does substring containment, so `"pgvector latency"` returns 0 matches on a page containing "pgvector achieves 12ms p50 latency". Section path already uses FTS5; extend the same tokeniser/BM25 path to page-grain keyword search so word order and whitespace stop mattering. Semantic mode masks this for `[semantic]` installs but the default install is the one users hit first.
- **Graceful fallback when embedding model fails to load** (from 1.11.0 LLM evaluation) — Auto-mode promises transparent keyword fallback when fastembed is unavailable, but only the `ImportError` (package missing) path falls back. If `fastembed` is installed and `_embedder.encode(...)` raises (offline machine, HF outage, restricted egress), the error propagates as a `ToolError` and the agent gets no result. Wrap `encode()`/`encode_query()` in the auto path with try/except, return keyword results with `search_mode: "keyword"`, `semantic_unavailable: true`, and a reason field. Optional: surface "semantic ready: yes/no" in `pdf_cache_stats` after a one-time boot probe.
- **Honest section titles on no-TOC documents** (from 1.11.0 LLM evaluation) — Heuristic section detector returns the first body line as `title` when no real heading is found, producing useless truncated-paragraph labels (often duplicated across sections). Either gate the title on a heading-vs-body signal (font size/weight from PyMuPDF spans, numbering/casing patterns) or return `title: null` when uncertain so the agent can choose how to render.
- **Trim `pdf_info.text_coverage` token cost** (from 1.11.0 LLM evaluation) — `text_coverage` currently returns one object per page; on a 500-page doc that's hundreds of nearly-identical objects in the agent's context. Options: ship summary stats (`pages_with_text`, `pages_with_only_images`, `scanned_page_ranges`), use a compact array (`text_chars_per_page: [...]`), or gate the per-page detail behind `pdf_info(detail=True)`.
- **Smaller LLM-evaluation polish items** (from 1.11.0 LLM evaluation) —
  - `mode="semantic"` without `[semantic]` installed silently returns `search_mode=None, matches=None`; should return an inline error payload with a `pip install 'pdf-mcp[semantic]'` hint, matching the page-spec error pattern.
  - `_resolve_path` raises `ToolError` on bad paths/URLs while page-spec errors return inline `{"error": ..., "hint": ...}`; unify on the inline-error pattern so agents have a single recovery path.
  - Document the inline-error contract (`status=OK` with an `error` field on parse failures) in tool descriptions so models reliably check `.error` before reading payload fields.
  - Consider a relevance threshold or explicit "low-confidence" flag for semantic/hybrid results — `score: 0.0159` matches on queries with no real answer are currently indistinguishable from real hits.
  - Two-column reading order is scrambled on some layouts; investigate PyMuPDF Pro / `pymupdf_layout` as an optional `pdf-mcp[layout]` extra (overlaps with the existing escalation item below).
