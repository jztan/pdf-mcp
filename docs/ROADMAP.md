...
- **Test suite:** 948 tests across unit, integration, and retrieval-quality benchmarks (944 fast + 4 `slow`). OCR tests skip cleanly when system Tesseract is absent. The `test_benchmark_*` files are fast unit tests for the benchmark scripts' helpers; billed/multi-minute checks (the LLM-judge coherence eval and the RRF v2 retrieval gate, both `slow`) are excluded from the release gate, which runs `pytest -m "not slow"`.
...
**Last Updated:** 2026-07-04 (v1.19.0 + v1.19.1 released same day — v1.19.0 never reached PyPI due to the pymupdf4llm 1.28 `find_tables` regression, v1.19.1 carries it. Merged to `develop` for 1.20.0 — **bbox/clip source geometry** on paragraph search hits and `pdf_read_pages` images/tables (+ per-page `page_rect`); **image xref dedup**. Test count 943→948)
