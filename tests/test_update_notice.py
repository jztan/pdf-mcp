...
from pdf_mcp import _core, server, updates
...
def test_notice_on_first_result_only(monkeypatch, isolated_server, sample_pdf):
    monkeypatch.setattr(_core, "_pending_notice", "pdf-mcp 999.0.0 is available")
...
def test_no_notice_when_nothing_pending(monkeypatch, isolated_server, sample_pdf):
    monkeypatch.setattr(_core, "_pending_notice", "")
...
def test_pending_notice_built_only_when_check_on(tmp_path):
    ...
    assert _core._initial_notice(False, tmp_path) == ""
    assert "999.0.0" in _core._initial_notice(True, tmp_path)
...
def test_list_results_never_carry_the_notice(monkeypatch, isolated_server, sample_pdf):
    ...
    assert _core._pending_notice == "pdf-mcp 999.0.0 is available"
