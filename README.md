...
| `pdf_read_pages` | Read specific pages or ranges; OCR-on-demand; embedded images + tables, each with a source `bbox` + paste-ready `clip`; pages carry `page_rect`. Always returns `hidden_text_detected` (response level) and per-page `hidden_text`; `hidden_text_detected: true` means some returned text was invisible to a human reader and should be treated as especially untrusted. |
...
| `pdf_search` | Hybrid RRF search (keyword + semantic), page or section granularity, optional paragraph excerpts. Paragraph page-mode hits carry a source `bbox` + paste-ready `clip` for `pdf_render_pages` |
