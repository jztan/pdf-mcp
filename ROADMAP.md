...
- **Normalise `total_matches` semantics across modes** (from 1.12.0 LLM evaluation) — `_escape_fts5_query` is tokenised in 1.12.0 but `get_fts_page_counts` still counts via `text.lower().count(query_lower)`. Result: a tokenised query like `pgvector latency` returns 4 matches with `total_matches: 0` because the literal phrase doesn't appear anywhere. Commit to evaluator's option 2: `total_matches == len(matches)` in every mode (keyword/semantic/hybrid). Update `get_fts_page_counts` to count token occurrences so `page_match_counts` keeps its per-page intensity signal for keyword mode. In hybrid/semantic, `page_match_counts` becomes `{page: 1}` per returned page — schema-stable, semantically thin, acceptable. Ship a property test alongside: `len(matches) == total_matches` across all modes and queries.
- **Relevance floor in hybrid mode** (from 1.12.0 LLM evaluation) — hybrid still returns top-k RRF results even when nothing is genuinely relevant (`"this string definitely does not appear"` → 5 matches with tiny RRF scores). Extend the `low_confidence` flag from semantic to hybrid, driven by the underlying semantic-component score. Surface a top-level `all_results_low_confidence: true` when every returned match crosses the threshold. Do NOT drop the matches — empty arrays are harder for an LLM to recover from than flagged-low results ("couldn't find it but here's the closest" vs "couldn't find it").
- **Section-title honesty for fallback cases** (from 1.12.0 LLM evaluation; partial fix of 1.11.0 finding #4) — top-1 chapter detection works now, but sub-section fallback still synthesises titles from body text ("Section 2: This paragraph discusses ..."). Return `title: null` when the heading detector can't find a real heading, and add a sibling `title_source: "toc" | "heading_detected" | null` field so an LLM can decide whether to display the title or fall back to "section at pages N–M".
