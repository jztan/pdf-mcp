...
for name, gt in GT.items():
        ...
        for q in r["questions"]:
            if q["kind"] == "y_axis_for_curve":
                ...
                for tag in ("left", "right"):
                    h = dict(hints); h[q["id"]] = tag
                    rr = v3.extract(pdf, 1, h)
                    for ch in rr["charts"]:
                        for c in ch.get("curves", []):
                            if str(c["style"]) != q["curve_style"] or \
                                    not c.get("points"):
                                continue
                ...
                hints[q["id"]] = gt["type"] if gt["type"] in ("line", "bar", "scatter") else "not_a_chart"
        r = v3.extract(pdf, 1, hints)
        r["_hints_used"] = hints
    ...
    if gt["type"] == "decline_expected":
        ok = not emitted
        rows.append((name, r["status"], "correctly-declined" if ok
                     else "WRONG-EMIT(decoy)", None))
        if not ok:
            wrong_emits += len(emitted); total_emitted_series += len(emitted)
        continue
    if not emitted:
...
print(f"\nEmitted-series accuracy: mean {np.mean(ok_errs):.2f}% of y-range "
      f"(n={len(ok_errs)})" if ok_errs else "no accurate emissions")
