...
### Changed
- Demo page v2.0.0 (versioned in step with the server from this release): interactive corpus flow (warm progress, overview triage cards, cross-document keyword search with the server's RRF fusion and term-coverage tie-break, per-document page grids, corpus-sized token scoreboard), bundled 5-PDF public-domain sample corpus, multi-file drop, and updated copy for semantic-by-default.
...
- `pdf_extract_chart`: extract exact `(x, y)` data tables from born-digital
  vector charts — values are read from the drawing geometry and calibrated
  against tick labels, not estimated. Every emitted chart carries a
  `verification_card` (the reading it made: axis scale/range/ticks and the
  series colour→label map) plus a per-reading `verify` flag on axes it is
  unsure about, so a vision-capable caller can confirm the reading against
  the returned render; ambiguous or unreadable charts decline with a
  rendered image and a reason. Dual-axis / chart-type ambiguity resolves via
  closed-enum hints, and a `p{n}.verify` verdict lets a caller confirm or
  correct a reading. Trust contract: coordinates are exact; readings are
  reliable on standard typography but not guaranteed, and auditable against
  the render ([#23](https://github.com/jztan/pdf-mcp/issues/23)).
- `pdf_read_pages(detect_charts=True)`: opt-in per-page `charts_detected`
  signal so agents notice extractable charts
