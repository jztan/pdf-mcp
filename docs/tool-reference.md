...
- **Column reliability**: each entry in `tables[]` also carries `columns_reliable` (bool), `false` when any cell in the table still holds two or more numbers, which happens on datasheets drawn without vertical rules and means the extractor could not split that table's columns.
...
- With `sections=True`, already-cached documents also get their section index backfilled at no budget cost, the same way. `docs` rows carry no per-doc section-coverage field (unlike `embeddings_cached`); the only way to check is a `pdf_search(granularity="section")` call.
