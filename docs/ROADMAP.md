...
- **Test suite:** 923 tests across unit, integration, and retrieval-quality benchmarks (919 fast + 4 `slow`). OCR tests skip cleanly when system Tesseract is absent. The `test_benchmark_*` files are fast unit tests for the benchmark scripts' helpers; billed/multi-minute checks (the LLM-judge coherence eval and the RRF v2 retrieval gate, both `slow`) are excluded from the release gate, which runs `pytest -m "not slow"`.
...
**Last Updated:** 2026-06-27 (v1.18.0 released; CJK keyword search fix and **content-trust / hidden-text detection both merged to `develop`** for 1.19.0 — content-trust adds the `content_trust.py` module, the `content_trust` block on `pdf_info`, and the `hidden_text_detected` flag on the read tools; synthetic benchmark 1.000/1.000, real-world spot-check passed; test count 890→923)
