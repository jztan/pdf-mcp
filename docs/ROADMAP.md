...
**Last Updated:** 2026-06-28 (v1.18.0 released; merged to `develop` for 1.19.0 — CJK keyword search fix; content-trust / hidden-text detection (`content_trust.py`, the `content_trust` block on `pdf_info`, `hidden_text_detected` on the read tools; synthetic benchmark 1.000/1.000); the **`pdf_search` hidden-text flag** completing the content-trust deferred follow-up; and a **cache TTL timezone fix** in `clear_expired()`. Test count 923→935)
