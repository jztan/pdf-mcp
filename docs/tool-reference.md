...
- `excerpt_style` (string, optional, default `"paragraph"`): `"paragraph"` returns the enclosing text block (adds `bbox`/`page_rect`/`clip`) where one can be located; `"snippet"` is a fixed-width context window; `"window"` returns the anchor block plus contiguous neighbours up to `window_tokens` and adds `window_blocks` and `anchor`, as in `pdf_search`.
- `window_tokens` (int, optional, default `600`): token budget per excerpt for `excerpt_style="window"`.
- `evidence_budget` (int, optional, `mode="auto"` only): opt-in per-query excerpt routing. The server picks the excerpt unit from how many documents hold an AND keyword match for the query (the count hybrid mode already computes): none (a paraphrase question) returns `"paragraph"` excerpts, exactly one (a single-document keyword question) returns one `"window"` of `evidence_budget` tokens per hit, two or more (the answer is spread across documents) returns `"snippet"` excerpts so more documents fit a fixed context budget. It overrides `excerpt_style` and `window_tokens`; document and page ranking are unchanged. Returns an inline `error` with `mode="keyword"` or `"semantic"` (their keyword count has different semantics or does not exist) and for a non-positive value. **Limitations:** snippet-routed hits carry no `bbox`/`page_rect`/`clip`, so callers that need geometry for `pdf_render_pages(clip=...)` should not pass it. The routing has no signal for boilerplate terms that appear in many documents (a query like "related work" routes to snippets). Measured on the 184-query corpus benchmark at a fixed 2,000-token budget with `evidence_budget=600` against the paragraph default: needle 0.839 vs 0.677 and spread 0.711 vs 0.600 (neither difference individually confirmed), trap 0.840 vs 0.760, described 0.217 vs 0.217 (unchanged by construction); against the Bedrock Knowledge Base anchor it is the one configuration ahead with confirmed margins on needle (+0.355) and spread (+0.289) and no confirmed loss.
...
- `excerpt_style`: the effective style: the echoed input, or the routed unit when `evidence_budget` is set.
- `allocation` (object, only with `evidence_budget`): `{"rule": "keyword_docs", "keyword_docs": N, "unit": "paragraph" | "window" | "snippet", "tokens": evidence_budget}`. Every match then also carries `unit` with the same value.
