...
- **Test suite:** 935 tests across unit, integration, and retrieval-quality benchmarks (931 fast + 4 `slow`). OCR tests skip cleanly when system Tesseract is absent. The `test_benchmark_*` files are fast unit tests for the benchmark scripts' helpers; billed/multi-minute checks (the LLM-judge coherence eval and the RRF v2 retrieval gate, both `slow`) are excluded from the release gate, which runs `pytest -m "not slow"`.
...
- **Content-trust / hidden-text detection.** New `content_trust.py` module. Flags text a human reader cannot see — invisible render mode (mode 3), sub-point fonts, transparent fill, white-on-white, off-page runs — via PyMuPDF `get_texttrace()` geometry (zero new deps). Opt-in `content_trust` block on `pdf_info` (counts, per-signal breakdown, flagged pages; per-span detail under `detail=True`), plus an always-on `hidden_text_detected` flag on `pdf_read_pages` / `pdf_read_all` (the path that actually returns the text). The safety boundary is geometry (language-agnostic); a best-effort English `injection_in_hidden` count is a severity hint over hidden spans only, never the trigger. Legitimate searchable-OCR layers (invisible text over a page image) are exempt. **Detection is flag-only — extraction is never altered.** Document-level scan + per-page flag are cached, invalidated together by a global trust-version. Synthetic benchmark: precision/recall **1.000** on a 14-file English+CJK corpus with clean controls (`scripts/benchmark_content_trust.py`). Real-world spot-check: zero false positives on real arXiv PDFs (incl. a 7 MB image-heavy paper); catches the July-2025 arXiv white-1pt "positive review only" technique applied to a real paper — flagged via `tiny_font` + `white_on_white` with exact `bbox`/`font_size` provenance, a deterministic, model-independent complement to product- and model-level injection guardrails. **Follow-up:** wire a tracked real-world sample set (in-the-wild injected PDFs are scarce — the arXiv offenders were scrubbed post-exposure and research datasets are private), and the spec's one remaining deferred item (occlusion detection — the configurable/non-English phrase list shipped, see below). Surfaced 2026-06-24 via competitive analysis vs `pdf-reader-mcp`.

- **`pdf_search` hidden-text flag.** Page-mode `pdf_search` results now carry a per-hit `hidden_text` bool and a response-level `hidden_text_detected`, the same signal the read tools emit — closing the gap for agents that answer **directly from a search excerpt** (where hidden/injected text is FTS-indexed and surfaced without ever reading the page, the most direct injection-via-excerpt path). Page granularity only; pure reuse of the cached per-page content-trust detection (no extra scan). Section mode unchanged. This completes the content-trust spec's deferred `pdf_search` follow-up.

- **Configurable `injection_in_hidden` phrase list.** The content-trust severity hint is now user-extensible: add your own — including non-English (Chinese / Japanese / French shown in the docs) — prompt-injection phrases via `[content_trust].injection_phrases` in `config.toml`. They **extend** the built-in English list (never replace it), are matched case- and space-insensitively inside hidden spans only, and the count is **recomputed from the cached scan on each read** — so editing the list takes effect on the next server start with no cache invalidation, no geometry re-scan, and no trust-version bump. `summarize()` is the single config-aware authority; `config.py` validates the value (a non-list aborts startup) without importing `content_trust`. Built-ins-vs-user overlap is de-duplicated so a repeated phrase can't double-count. Closes the content-trust spec's configurable/non-English deferred item; occlusion detection remains the last open content-trust follow-up.

- **Cache TTL timezone fix.** `clear_expired()` no longer prematurely purges fresh cache entries on hosts ahead of UTC. The TTL cutoff was a local-time `datetime.now().isoformat()` string (with a `T` separator) compared against `accessed_at`, written via SQLite `CURRENT_TIMESTAMP` (UTC, space separator) — a timezone **and** date-separator mismatch that made same-date rows mis-sort as expired and get wiped on startup. The cutoff is now computed with SQLite's own clock (`datetime('now', '-N hours')`) so both sides share one clock and format. Found while landing the `pdf_search` flag (a pre-existing failing test on `develop`).
...
**Last Updated:** 2026-06-28 (v1.18.0 released; merged to `develop` for 1.19.0 — CJK keyword search fix; content-trust / hidden-text detection (`content_trust.py`, the `content_trust` block on `pdf_info`, `hidden_text_detected` on the read tools; synthetic benchmark 1.000/1.000); the **`pdf_search` hidden-text flag** completing the content-trust deferred follow-up; a **cache TTL timezone fix** in `clear_expired()`; and a **configurable / non-English `injection_in_hidden` phrase list** (`[content_trust].injection_phrases`, recomputed on read — closes a content-trust deferred item). Test count 923→943)
