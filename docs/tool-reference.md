...
- **Column reliability**: each entry in `tables[]` also carries `columns_reliable` (bool), `false` when any cell in the table still holds two or more numbers, which happens on datasheets drawn without vertical rules and means the extractor could not split that table's columns, or when the header row holds money amounts, which means the real header sits outside the detected table and the first row returned as `header` is data. A single value in a cell merged across several columns is placed under the column it is printed in.
...
- **Column reliability caveat**: `columns_reliable` is a **table-level caution, not a per-row verdict**: an individual row in a flagged table may still be perfectly aligned, and a row in a table flagged `true` is not thereby verified. When `columns_reliable` is `false`, render the table with `pdf_render_pages(clip=...)` (each table carries a `clip`) to read the columns off the page instead of trusting a merged cell.
- `content_warning` (string).
...
- **Hit context** (page mode, conditional): `section_path` (list of strings, root to leaf, at most 3) names the PDF's own outline entries enclosing the hit, and `lead_in` (string) is the sentence printed directly above the hit's table or list that introduces it, ending in a colon (for example *"Following are the supplemental consolidated financial results ... on an unaudited pro forma basis ...:"*). Together they tell a segment table from the consolidated statement, or pro forma figures from actual ones, when the excerpt alone is a bare table row. Both are absent, not null, when they do not apply. **Limitations:** `section_path` comes from the document outline only, so documents without one never carry it (detected headings are too noisy on financial filings to show as labels). When several outline entries start on the hit's page, the heading printed nearest above the hit wins; a hit with no `bbox` stops the path above a level that has two headings on that page. The last outline entry at each level is assumed to cover at most two pages past its start, so back matter the outline does not list (references, appendices without an entry) carries no path rather than the last section's. `lead_in` is absent for prose hits, and it crosses only short unit or period bands (`(In millions)`, `Year Ended June 30,`): a data row between the sentence and the hit stops it, so a hit several rows down a long table may not carry it.
...
- For text extraction the budget is checked between docs, not within one, so a very large PDF's extraction can push the call past `budget_seconds`. Embedding work is finer-grained: it commits in page batches with the clock checked between batches, so embedding overshoot is bounded by one batch (a few seconds).
...
- `excerpt_style` (string, optional, default `"paragraph"`): `"paragraph"` returns the enclosing text block (adds `bbox`/`page_rect`/`clip`) where one can be located; `"snippet"` is a fixed-width context window; `"window"` returns the anchor block plus contiguous neighbours up to `window_tokens` and adds `window_blocks` and `anchor`, as in `pdf_search`; `"auto"` (`mode="auto"` only) lets the server pick the unit per query from how many documents hold an AND keyword match for the query: none (a paraphrase question) routes to `"paragraph"`, exactly one (a single-document keyword question) routes to one `"window"` of `window_tokens` per hit, two or more (the answer is likely spread) routes to `"snippet"` so more documents fit a fixed context budget. Use `"auto"` when you do not know whether the answer lives in one document or many. The choice is explained in the response's `excerpt_routing`; document and page ranking are unchanged. Returns an inline `error` with `mode="keyword"` or `"semantic"`. Snippet-routed hits carry `bbox`/`page_rect`/`clip` covering the **union of the text blocks the excerpt spans**, and every emitted bbox is **validated by re-extracting the text under it**: the excerpt must actually appear in the crop, so a `pdf_render_pages(clip=...)` crop shows what the excerpt shows. When only part of the excerpt can be validated the hit carries `geometry: "partial"` alongside the bbox; when nothing validates it carries `geometry: "unlocatable"` and no bbox (measured on the benchmark corpus: 97.2% validated bbox, 2.8% unlocatable, 0 unvalidated emissions). The explicit legacy `"snippet"` style remains geometry-free. Snippet excerpts for pages without a keyword hit open at the sentence or heading boundary nearest their matched content. **Limitations:** the routing has no signal for boilerplate terms that appear in many documents (such a query routes to snippets). Measured on the 184-query corpus benchmark at a fixed 2,000-token budget (as `P-auto`/`P-autoP`, window 600) against the paragraph default: needle 0.839 vs 0.677 and spread 0.711 vs 0.600 (neither difference individually confirmed), trap 0.840 vs 0.760, described unchanged by construction; against the Bedrock Knowledge Base anchors it is the one configuration with confirmed margins on needle and spread against both B0 and B1 and no confirmed loss.
...
**Returns:**
- `matches` (array): cross-document hits in fused order, each `{path, doc_title, page, excerpt, position, source, hidden_text}`, plus `section_path` and `lead_in` when they apply (same rules as `pdf_search`), plus `bbox`/`page_rect`/`clip` when `excerpt_style` is `"paragraph"` or `"window"` (`"window"` also carries `window_blocks` and `anchor`).
...
- `excerpt_style`: the effective style: the echoed input, or the routed unit when `excerpt_style="auto"` was requested.
- `excerpt_routing` (object, only for `excerpt_style="auto"`): `{"unit", "reason", "keyword_doc_count", "matching_doc_count", "window_tokens_applied"}`. `reason` is a human-readable explanation of the routing choice; `keyword_doc_count` is the AND-keyword document count the rule keyed on (it can be smaller than `matching_doc_count`, which is the size of `doc_match_counts` and, in hybrid mode, includes documents only the semantic arm found); `window_tokens_applied` is `null` unless the window branch actually fired.
- `coverage` (object): `{"searched": docs actually queried, "corpus": total resolved files}`.
- `low_text_coverage` (object): `{path: "partial" | "none"}` for every searched doc whose pages are not all extractable; `{}` when they all are. A doc listed here contributes little or nothing to the searchable text, so zero hits from it mean *unknown*, not "absent" — its scanned pages need `pdf_read_pages(ocr=True)` before any search can see them.
...
#       "ocr": {"available": true, "description": "Opt-in: ... only when pdf_read_pages is called with ocr=true. ..."}
#     },
#     "search": {
