...
"""
import glob, os, time, importlib.util
...
def run():
    ...
    unlabeled = [k for k in flags if k not in KNOWN_CHART_PAGES
                 and k not in KNOWN_NON_CHART_PAGES]

    times_ms = sorted(t*1000 for t in times)
    print(f"pages swept: {n_pages}   flagged: {len(flags)} "
          f"({100*len(flags)/n_pages:.1f}%)")
    print(f"runtime/page: median {times_ms[len(times_ms)//2]:.1f}ms  "
    ...
    print(f"false positives on known non-chart pages: "
